...
// 鼠标操作的回调函数
void onMouse1(int event, cv::Mat frame, std::vector<cv::Rect>& boundingboxes, void *para)
{
	switch (event)
	{
	case cv::EVENT_RBUTTONDOWN:
		cv::selectROIs("MultiTracker", frame, boundingboxes);
	default:
		break;
	}
...
mousepara* MouseEvent1(int event, int x, int y, int flags, void *para)
{
